...
def writeTable(fileName, words, translate):
    ...
    for (untranslated, contracted) in words:
        contracted = smart_unicode(contracted).replace(DUMMY_TEXT, '') # remove the DUMMY_TEXT from the braille as the translation doesn't contain it anyway
        if translate(smart_unicode(untranslated)) != contracted:
